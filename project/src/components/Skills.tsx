  ...
  return (
    <div className="mb-4">
      <div className="flex justify-between items-center mb-1.5">
        ...
        <span className={`${textColorMap[color]} text-xs font-mono font-semibold`}>{level}%</span>
      </div>
...
export default function Skills() {
  ...
  return (
    <section id="skills" ref={ref as React.RefObject<HTMLElement>} className="py-24 px-6 relative">
      <div className="absolute inset-0 bg-gradient-to-b from-transparent via-sky-950/5 to-transparent pointer-events-none" />
      <div className="max-w-6xl mx-auto">
        <SectionHeader title="Skills" sub="Technologies I work with" inView={inView} />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-12">
          {skillGroups.map((group, i) => (
            ...
            >
              <h3 className={`${textColorMap[group.color]} font-semibold text-sm font-mono uppercase tracking-wider mb-5`}>
                {group.label}
              </h3>
              {group.skills.map((s) => (
...
export function SectionHeader({ title, sub, inView }: { title: string; sub: string; inView: boolean }) {
  return (
    <div className={`text-center transition-all duration-700 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
      <span className="text-sky-400 font-mono text-sm tracking-widest uppercase">{sub}</span>
      <h2 className="text-3xl sm:text-4xl font-bold text-white mt-2 mb-3">{title}</h2>
